...
discord.ChannelType.text, discord.ChannelType.public_thread, discord.ChannelType.private_thread,
...
async def on_message(new_msg):
    ...
    if (
        ...
        id in ALLOWED_CHANNEL_IDS for id in (new_msg.channel.id, getattr(new_msg.channel, "parent_id", None))))
            or (ALLOWED_ROLE_IDS and (new_msg.channel.type == discord.ChannelType.private or not any(
        role.id in ALLOWED_ROLE_IDS for role in new_msg.author.roles)))
    ...
    while curr_msg and len(reply_chain) < MAX_MESSAGES:
        async with msg_nodes.setdefault(curr_msg.id, MsgNode()).lock:
                ...
                try:
                    ...
                    ([m async for m in curr_msg.channel.history(before=curr_msg, limit=1)] or [None])[0])
                        ...
                        if next_msg_id := curr_msg.channel.id if next_is_thread_parent else getattr(curr_msg.reference,
                            ...
                            curr_node.next_msg = (
                                (curr_msg.channel.starter_message or await curr_msg.channel.parent.fetch_message(
                                    next_msg_id))
                                if next_is_thread_parent
                                else (curr_msg.reference.cached_message or await curr_msg.channel.fetch_message(
                                    next_msg_id))
                            )
            ...
            if curr_node.fetch_next_failed or (curr_node.next_msg and len(reply_chain) == MAX_MESSAGES):
                user_warnings.add(
                    f"⚠️ Only using last {len(reply_chain)} message{'' if len(reply_chain) == 1 else 's'}")
        ...
        async with new_msg.channel.typing():
            async for curr_chunk in await acompletion(**kwargs):
                if prev_chunk:
                    ...
                    is_final_edit: bool = curr_chunk.choices[0].finish_reason != None or len(
                        response_contents[-1] + curr_content) > EMBED_MAX_LENGTH

                    if is_final_edit or ((
                                                 not edit_task or edit_task.done()) and dt.now().timestamp() - last_task_time >= EDIT_DELAY_SECONDS):
